...
#if ONESTEPFCN==0
#error Separate output and update functions are not supported by rt_main.c. You must update rt_main.c to suit your application needs, or select the 'Single output/update function' option.
#endif

#if TERMFCN==0
#error The terminate function is required by rt_main.c. You must update rt_main.c to suit your application needs, or select the 'Terminate function required' option.
#endif

#if MULTI_INSTANCE_CODE==1
#error The static version of rt_main.c does not support reusable code generation. Either deselect ERT option 'Generate reusable code', select option 'Generate an example main program', or modify rt_main.c for your application needs.
#endif
...
/*==========================================*
 * External mode as background thread patch *
 *==========================================*/
#if EXT_MODE==1
#include <thread>
#include <mutex>
...
std::mutex mtx;
std::condition_variable main_step_complete;
bool external_mode_thread_step_done{ true };

bool model_is_running{ true };

void external_mode_background_thread()
{
  while (model_is_running) {
    std::unique_lock<std::mutex> lck(mtx);
    main_step_complete.wait(lck);
    external_mode_thread_step_done = false;

    /* external mode */
    rtExtModeOneStep(rtmGetRTWExtModeInfo(RT_MDL),
                     NUMST,
                     (boolean_T *)&rtmGetStopRequested(RT_MDL));
    external_mode_thread_step_done = true;
  }
}
...
int_T main(int_T argc, const char *argv[])
{
  ...
  std::thread ext_mode_t(external_mode_background_thread);
  ...
  while (rtmGetErrorStatus(RT_MDL) == NULL &&
    ...
    if (external_mode_thread_step_done) {
      main_step_complete.notify_one();
    }
