...
#define SERVER_PORT_PRINTF(FORMAT, ARG1) fprintf(stdout, FORMAT, ARG1); fflush(stdout)
#endif
...
typedef enum { VERBOSITY_LEVEL_0=0, VERBOSITY_LEVEL_1, VERBOSITY_LEVEL_2 }
  VerbosityLevel;
...
typedef struct ConnectionData_tag {
  ...
  VerbosityLevel verbosity;
        // enum indicating the level of verbosity to be displayed on the output
...
static int serverStreamRecv(
  ...
  size_t size,
  size_t * sizeRecvd);
static SOCKET serverOpenSocket(int port, char * serverInfoFile, CommsProtocol
  protocol,int udpSendBufSize, int udpRecvBufSize);

#if (!defined(VXWORKS))

static SOCKET clientOpenSocket(char * hostName, unsigned int portNum,
  CommsProtocol protocol, int udpSendBufSize, int udpRecvBufSize);
...
static UDPPacketBuffer * createUDPPacketBuffer(int maxPacketSize)
{
  ...
  UDPPacketBuffer * udpPacketBuffer = (UDPPacketBuffer *) malloc(sizeof
    (UDPPacketBuffer));
  ...
  protocol, int udpSendBufSize, int udpRecvBufSize)
{
  ...
  int sockStatus;
  rtiostream_socklen_t sFdAddSize = (rtiostream_socklen_t) sizeof(struct
    sockaddr_in);
  SOCKET lFd;
  int option;
  ...
  if (lFd == INVALID_SOCKET) {
    ...
    if (protocol == UDP_PROTOCOL) {
      if (lFd != INVALID_SOCKET) {
        ...
        //  transfer rate
        sockStatus =
          setsockopt(lFd, SOL_SOCKET, SO_SNDBUF,(char*)&udpSendBufSize, sizeof
                     (int));
        if (sockStatus == SOCK_ERR) {
      ...
      if (lFd != INVALID_SOCKET) {
        ...
        //  possibility of buffer overflow
        sockStatus =
          setsockopt(lFd, SOL_SOCKET, SO_RCVBUF,(char*)&udpRecvBufSize, sizeof
                     (int));
        if (sockStatus == SOCK_ERR) {
    ...
    if (port == 0) {
      ...
      if (sockStatus == SOCK_ERR) {
        ...
        if (serverInfoFile != NULL) {
          ...
          if (fh == NULL) {
            fprintf(stderr,
                    "Unable to open output file to write server port number: %s\n",
                    strerror(errno));
            lFd = INVALID_SOCKET;
          }
...
static void serverAcceptSocket(ConnectionData * connection)
{
  ...
  if (connection->serverData->listenSock == INVALID_SOCKET) {
    connection->serverData->listenSock = serverOpenSocket(connection->
      serverData->port,
      connection->serverData->serverInfoFile,
      connection->protocol,
  ...
  while (count < argc) {
    ...
    if (option != NULL) {
      ...
      } else if ((strcmp(option, "-recv_timeout_secs") == 0) && (count != argc))
      {
        ...
        count++;
        itemsConverted = sscanf(timeoutSecsStr,"%d%1s", (int *) recvTimeout,
          tmpstr);
        if (itemsConverted != 1 ) {
          retVal = RTIOSTREAM_ERROR;
        } else {
      ...
      } else if ((strcmp(option, "-server_info_file") == 0) && (count != argc))
      {
        ...
        *isUsingSeqNum = 0;
        ...
        } else if (strcmp(protocolStr, UDP_PACKET_LOSS_DETECTON_PROTOCOL_STRING)
                   == 0) {
          *protocol = UDP_PROTOCOL;
      ...
      } else if ((strcmp(option, "-udpsendbuffersize") == 0) && (count != argc))
      {
        ...
        count++;
        itemsConverted = sscanf(udpSendBufSizeStr,"%d%1s", udpSendBufSize,
          tmpstr);
        if (itemsConverted != 1 ) {
          retVal = RTIOSTREAM_ERROR;
        } else {
      ...
      } else if ((strcmp(option, "-udpreceivebuffersize") == 0) && (count !=
                  argc)) {
        char tmpstr[2];
        int itemsConverted;
        const char *udpRecvBufSizeStr = (char *)argv[count];
        count++;
        itemsConverted = sscanf(udpRecvBufSizeStr,"%d%1s", udpRecvBufSize,
          tmpstr);
        if (itemsConverted != 1 ) {
          retVal = RTIOSTREAM_ERROR;
        } else {
  ...
  CommsProtocol protocol, int udpSendBufSize, int udpRecvBufSize)
{
  ...
  if (protocol == UDP_PROTOCOL) {
    if (errStatus!=RTIOSTREAM_ERROR) {
      ...
      //  transfer rate
      int sockStatus =
        setsockopt(cSock, SOL_SOCKET, SO_SNDBUF,(char*)&udpSendBufSize, sizeof
                   (int));
      if (sockStatus == SOCK_ERR) {
        printf("setsocketopt() SO_SNDBUF call failed.\n");
    ...
    if (errStatus!=RTIOSTREAM_ERROR) {
      ...
      //  possibility of buffer overflow
      int sockStatus =
        setsockopt(cSock, SOL_SOCKET, SO_RCVBUF,(char*)&udpRecvBufSize, sizeof
                   (int));
      if (sockStatus == SOCK_ERR) {
        printf("setsocketopt() SO_RCVBUF call failed.\n");
        cSock = INVALID_SOCKET;
...
#define TMP_BUF_SIZE                   (40)
  ...
  do {
    retVal = serverStreamRecv(connection, (void *) tmpBuf, TMP_BUF_SIZE,
      &numRecvd);
...
int rtIOStreamOpen(int argc, void * argv[])
{
  ...
  if (result != RTIOSTREAM_ERROR) {
       // polyspace DEFECT:USELESS_IF [Not a defect:Unset] "Needed for Windows"
    if (isClient == 1) {

#if (!defined(VXWORKS))      // Client side connection not supported on VxWorks

      sock = clientOpenSocket(xHostName, xPortNum, protocol,udpSendBufSize,
        udpRecvBufSize);
      ...
      sock = serverOpenSocket((int)xPortNum, serverInfoFile, protocol,
        udpSendBufSize,udpRecvBufSize);
      if (sock == INVALID_SOCKET) {
        result = RTIOSTREAM_ERROR;
      }
    }
