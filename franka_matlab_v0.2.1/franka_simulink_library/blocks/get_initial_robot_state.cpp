...
#define S_FUNCTION_NAME get_initial_robot_state
...
static void mdlInitializeSizes(SimStruct* S) {
  ...
  std::vector<std::string> output_signals_init_;
  
  signalsStringToSignalsVector(output_signals_init_, output_singals_str);
  ...
  if (!ssSetNumOutputPorts(S, output_signals_init_.size()))
    return;
  for (int i = 0; i < output_signals_init_.size(); i++) {
    ssSetOutputPortWidth(S, i, getWidthForSignalString[output_signals_init_[i]]);
...
static void mdlRTW(SimStruct *S)
{
    ...
    char robot_id_cstr[robot_id_string.size() + 1];
