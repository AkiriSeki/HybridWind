...
function cs = config_franka_emika_core(cs)
% MATLAB function for configuration set generated on 01-Mar-2021 14:01:00
...
% Original configuration set target is franka_emika_panda.tlc
try
    cs.switchTarget('franka_emika_panda.tlc', '');
catch ME
    disp(ME.message);
...
cs.set_param('ProdHWDeviceType', 'Intel->x86-64 (Linux 64)');   % Production device vendor and type
...
cs.set_param('CustomCodeGlobalsAsFunctionIO', 'off');   % Enable custom code globals as function interface
cs.set_param('GPUAcceleration', 'off');   % GPU acceleration
cs.set_param('SimTargetLang', 'C++');   % Language
cs.set_param('SimDLTargetLibrary', 'mkl-dnn');   % Target library
...
cs.set_param('Toolchain', 'Franka Simulink | gmake (64-bit linux)');   % Toolchain
...
cs.set_param('ParameterMemberVisibility', 'private');   % Parameter visibility
cs.set_param('GenerateParameterAccessMethods', 'None');   % Parameter access
cs.set_param('InternalMemberVisibility', 'private');   % Internal data visibility
...
cs.set_param('GenerateExternalIOAccessMethods', 'Inlined structure-based method');   % External I/O access
cs.set_param('ExternalIOMemberVisibility', 'private');   % External I/O visibility
...
cs.set_param('ArrayContainerType', 'C-style array');   % Array container type
