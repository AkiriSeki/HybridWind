...
#define S_FUNCTION_NAME get_model
...
static void mdlInitializeSizes(SimStruct* S) {
  ...
  if (!ssSetNumInputPorts(S, numInputs))
    return;
  ...
  // pose array double 16
  ssSetOutputPortWidth(S, 0, 16);
  ssSetOutputPortDataType(S, 0, SS_DOUBLE);

  // body jacobian double 42
  ssSetOutputPortWidth(S, 1, 42);
  ssSetOutputPortDataType(S, 1, SS_DOUBLE);

  // zero jacobian double 42
  ssSetOutputPortWidth(S, 2, 42);
  ssSetOutputPortDataType(S, 2, SS_DOUBLE);

  // mass matrix double 1
  ssSetOutputPortWidth(S, 3, 49);
  ssSetOutputPortDataType(S, 3, SS_DOUBLE);

  // coriolis double 7
  ssSetOutputPortWidth(S, 4, 7);
  ...
  ssSetDWorkRTWStorageClass(S, 0, SS_RTW_STORAGE_AUTO);
