...
function cs = config_franka_emika_shrlib_core( cs )
% MATLAB function for configuration set generated on 14-Feb-2020 18:05:58
...
% Original configuration set target is franka_emika_panda_shrlib.tlc
try
    cs.switchTarget('franka_emika_panda_shrlib.tlc', '');
catch ME
    disp(ME.message);
...
cs.set_param('ProdEqTarget', 'off');   % Test hardware is the same as production hardware
cs.set_param('TargetHWDeviceType', 'MATLAB Host');   % Test device vendor and type
cs.set_param('TargetLargestAtomicInteger', 'Char');   % Largest atomic integer size in test hardware
cs.set_param('TargetLargestAtomicFloat', 'None');   % Largest atomic floating-point size in test hardware
...
cs.set_param('ERTCustomFileTemplate', 'franka_emika_panda_file_process.tlc');   % File customization template
